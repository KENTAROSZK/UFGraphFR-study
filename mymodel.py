...
class UFGraphFR(torch.nn.Module):
    def __init__(
        ...
        self.embedding_user = (
            torch.nn.Linear(in_features=self.embed_dim, out_features=self.latent_dim)
            if config["use_jointembedding"]
            else torch.nn.Embedding(num_embeddings=1, embedding_dim=self.latent_dim)
        )
        ...
        self.user_mlp = CommonMLP(
            input_dim=input_dim,
            output_dim=self.latent_dim,
            layers=[input_dim, self.latent_dim * 2],
        )
    ...
    def forward(self, x: torch.Tensor, uef: torch.Tensor = None):
        ...
        if self.config["use_jointembedding"]:
            ...
            user_embedding = self.embedding_user(uef)
...
class MultiheadAttention(nn.Module):
    # n_heads：多头注意力的数量
    # hid_dim：每个词输出的向量维度
    def __init__(self, hid_dim, num_heads, dropout=0.1, use_cuda=False, use_mps=False):
        ...
        # 缩放
        self.scale = (
            torch.sqrt(torch.FloatTensor([self.hid_dim // self.n_heads])).to(
                torch.device("cuda")
            )
            if use_cuda
            else torch.sqrt(torch.FloatTensor([self.hid_dim // self.n_heads]))
        )
        if use_cuda:
...
class TransformerBlockKan(nn.Module):
    def __init__(
        self, input_dim, output_dim, dropout=0.1, use_cuda=False, use_mps=False
    ):
        super(TransformerBlockKan, self).__init__()
        self.attention = MultiheadAttention(
            hid_dim=input_dim, num_heads=8, use_cuda=use_cuda, use_mps=use_mps
        )
