...
Open Acade        # 将 self.user_infos 中 的 NaN 元素替换成 " "
        self.user_infos = self.user_infos.fillna("")

        # self.item_infos = item_infos
        self.dataset = config['dataset']mmunity License V1
"""
...
class EmbeddingUtils:
    ...
    def __init__(self, config, user_infos, dataset):
        ...
        if config["pre_model"] == "USE":
            ...
            options = TextEmbedderOptions(
                base_options=BaseOptions(model_asset_path=model_path), quantize=True
            )
    ...
    def embedding_users(self, user_id):
        ...
        if self.dataset == "ml-1m":
            ...
            if len(user) == 0:
                prompts = f"The user'id is {user_id}"
            else:
                prompts = "The user'id is {} and his gender is {}, he is {} years old, works in the field of {} and lives at zip code {}".format(
                    user["uid"].iloc[0],
                    user["gender"].iloc[0],
                    user["age"].iloc[0],
                    user["occupation"].iloc[0],
                    user["zipcode"].iloc[0],
                )
            ...
            if len(user) > 0:
                for i in range(len(sx)):
                    prompts += mb.format(sx[i], user[sx[i]].iloc[0])
            else:
                prompts = f"The user'id is {user_id}"
            ...
            if len(user) == 0:
                ...
                prompts = "The user'id is {} and his is from {}, he join the douban at {}, his self-introduction is {}.".format(
                    user["uid"].iloc[0],
                    user["living_place"].iloc[0],
                    user["join_time"].iloc[0],
                    user["self_statement"].iloc[0],
                )
            ...
            if len(user) == 0:
                ...
                prompts = "The user'id is {} and his look {} items".format(
                    user["uid"].iloc[0], user["tag"].iloc[0]
                )
        ...
        if not prompts or prompts.strip() == "":
            prompts = f"User {user_id} information not available"
