...
def construct_user_relation_graph_via_user(round_user_params, latent_dim, similarity_metric):
    ...
    user_embedding = np.zeros((len(round_user_params), latent_dim*100), dtype='float32')
    for user in round_user_params.keys():
...
def MP_on_graph_with_embedding_user(round_user_params, item_num, latent_dim, topk_user_relation_graph, layers):
    # prepare the item embedding array.
    user_embedding = np.zeros((len(round_user_params), 100*latent_dim), dtype='float32')
    for user in round_user_params.keys():
        user_embedding[user] = round_user_params[user]['embedding_user.weight'].numpy().flatten()
    ...
    user_embedding_dict = {}
    for user in round_user_params.keys():
        user_embedding_dict[user] = torch.from_numpy(aggregated_item_embedding[user].reshape( latent_dim,100))
